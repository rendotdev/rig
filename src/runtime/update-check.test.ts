...
import { NpmUpdateCheckService } from "./update-check";
...
describe("npm update checks", () => {
  test("skips checks when disabled", async () => {
    ...
    const service = new NpmUpdateCheckService({
      homeDir: home,
      fetch: async () => {
  ...
  test("fetches, caches, and renders newer npm versions", async () => {
    ...
    expect(await readFile(new RigPaths({ homeDir: home }).updateCheckCachePath, "utf8")).toContain(
      '"latestVersion": "1.2.4"',
    );

    const cached = new NpmUpdateCheckService({
      homeDir: home,
      now: () => 1001,
      packageName: "@scope/tool",
      fetch: async () => {
    ...
    await writeFile(
      new RigPaths({ homeDir: home }).updateCheckCachePath,
      '{"checkedAt": 1002,"latestVersion":"1.2"}\n',
      "utf8",
    );
    const shorter = new NpmUpdateCheckService({
      homeDir: home,
      now: () => 1003,
      packageName: "@scope/tool",
      fetch: async () => {
  ...
  test("ignores stale cache misses and invalid registry data", async () => {
    ...
    const missing = new NpmUpdateCheckService({
      homeDir: home,
      now: () => 10_000_000,
      cacheTtlMs: 1,
    ...
    const invalid = new NpmUpdateCheckService({
      homeDir: home,
      now: () => 10_000_000,
      cacheTtlMs: 1,
    ...
    const invalidCache = new NpmUpdateCheckService({
      homeDir: home,
  ...
  test("handles invalid cache files and fetch failures", async () => {
    ...
    const throwing = new NpmUpdateCheckService({
      homeDir: home,
      fetch: async () => {
    ...
    const aborted = new NpmUpdateCheckService({
      homeDir: home,
      timeoutMs: 1,
