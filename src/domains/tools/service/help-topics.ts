...
export class HelpTopicsService extends Context.Service<
  HelpTopicsService,
  {
    readonly list: Effect.Effect<string[]>;
    readonly render: (name: string) => Effect.Effect<string | undefined>;
    readonly renderList: Effect.Effect<string>;
  }
>()("@rendotdev/rig/tools/presentation/HelpTopicsService", {
  make: Effect.gen(function* () {
    const list = Effect.sync(() => Object.keys(TOPICS)).pipe(
      Effect.withSpan("HelpTopicsService.list"),
    );
    const render = Effect.fn("HelpTopicsService.render")(function* (name: string) {
      const topic = TOPICS[name];
      return topic ? `# Help \u2022 ${topic.title}\n\n${topic.content}` : undefined;
    });
    const renderList = Effect.sync(() => {
      const lines = ["# Help Topics", "", "Usage: `rig help <topic>`", ""];
      for (const [name, topic] of Object.entries(TOPICS)) {
        lines.push(`  ${name.padEnd(14)} ${topic.title}`);
      }
      return lines.join("\n");
    }).pipe(Effect.withSpan("HelpTopicsService.renderList"));
    return { list, render, renderList } as const;
  }),
}) {
  static readonly layer = Layer.effect(HelpTopicsService, HelpTopicsService.make);
}

export const helpTopicsLayer = HelpTopicsService.layer;
