...
type ToolRunContext<Input, Env = unknown, Collections = Record<string, CollectionHandle>> = {
  input: Input;
  env: Env;
...
type ToolCollectionHandles<Collections extends ToolCollectionDefinitions> = {
  [Name in keyof Collections]: Collections[Name] extends CollectionDefinition<infer Schema>
    ? CollectionHandle<z.output<Schema>>
    : CollectionHandle;
};

type ToolCommandBuilder<Env, Collections> = <Input extends RigSchema, Output extends RigSchema>(
...
type ToolCommandsFactory<Env, Collections, Commands extends ToolCommandMap> = (
