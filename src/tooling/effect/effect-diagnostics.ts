...
async function runEffectDiagnostics(): Promise<void> {
  const executable = (await capture(["bunx", "effect-tsgo", "get-exe-path"])).trim();
  if (!executable) throw new Error("@effect/tsgo did not report its diagnostics executable.");
  if (process.platform !== "win32") await execute(["chmod", "+x", executable]);
  await execute([
    "bunx",
    "effect-tsgo",
    "diagnostics",
...
export const effectDiagnostics = await runEffectDiagnostics();
