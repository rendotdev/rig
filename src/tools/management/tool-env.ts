...
class ToolEnvTargetClass {
  constructor(readonly tool: string) {}

  parse(value: string): ToolEnvTargetClass {
    if (value && !value.includes(".")) return new ToolEnvTargetClass(value);
    throw new RigErrorClass("INPUT_ERROR", `Env target must use <tool>: ${value}`);
  }
}

const toolEnvTarget = new ToolEnvTargetClass("");

class EnvKeyParserClass {
  ...
  private parseOne(key: string): string {
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
      throw new RigErrorClass("INPUT_ERROR", `Env key must be a valid shell variable name: ${key}`);
    }
    return key;
  }
}

class EnvAssignmentParserClass {
  ...
  private parseOne(assignment: string): [string, string] {
    ...
    if (separator <= 0 || !/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
      throw new RigErrorClass("INPUT_ERROR", `Env assignment must use KEY=VALUE: ${assignment}`);
    }
    return [key, assignment.slice(separator + 1)];
  }
}

class DotEnvDocumentClass {
  ...
  private parseLine(
    ...
    if (!match) {
      throw new RigErrorClass("TOOL_INVALID", "Invalid .env line.", { path, line: lineNumber });
    }
...
export class ToolEnvServiceClass {
  private readonly loader: ToolLoaderClass;
  private readonly assignments = new EnvAssignmentParserClass();
  private readonly keys = new EnvKeyParserClass();
  private readonly document = new DotEnvDocumentClass();
  private readonly summary = new EnvSchemaSummaryClass();
  ...
  private validateEnv(
    ...
    throw new RigErrorClass("TOOL_INVALID", `Tool ${target.tool} env would be invalid.`, {
      path: envPath,
