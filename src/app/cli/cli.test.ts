...
class CliHarness {
  ...
  async run(args: string[]): Promise<string> {
    ...
    const layer = cliCommandLayer.pipe(
      Layer.provide(Layer.merge(RigPathsOptionsConfigService.layer, RigPathsPlatformService.layer)),
    );
    await Effect.runPromise(
      CliCommandService.use((service) => service.run(["node", "rig", ...args])).pipe(
...
describe("cli application", () => {
  ...
  test("prints update notices from default status", async () => {
    const home = await workspaces.create();
    const rigDirectory = join(home, "rig");
    const updateCheckCachePath = join(rigDirectory, "update-check.json");
...
`,
  ...
  test("renders typed command failures through the CLI platform", async () => {
    const home = await workspaces.create();
    const cli = new CliHarness(home);

    await cli.run(["help", "missing"]);
    ...
    await cli.run(["run", "sample", "x"]);
    expect(process.exitCode).toBe(1);
    expect(cli.errorOutput).toContain("Command id must use <tool>.<command>: sample");
    await cli.run(["create", "sample"]);
    cli.logs.length = 0;
    await cli.run(["run", "sample.example", "text=x", "--as", "bad.id"]);
    expect(process.exitCode).toBe(1);
    expect(cli.errorOutput).toContain("Pipeline id is invalid: bad.id");
    await cli.run(["run", "sample.example", "text=x", "--query", "data.missing"]);
    expect(process.exitCode).toBe(1);
    expect(cli.errorOutput).toContain("Query is missing: data.missing");
    await cli.run(["run", "sample.example", "text=x", "--query", "data.text.extra"]);
    expect(process.exitCode).toBe(1);
    expect(cli.errorOutput).toContain("Query cannot access: data.text.extra");

    await Effect.runPromise(
      CliPlatformService.use((service) => service.fail(new Error("plain failure"))).pipe(
        Effect.provide(cliPlatformLayer),
      ),
    );
    expect(process.exitCode).toBe(1);
