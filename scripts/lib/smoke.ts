...
export class SmokeScriptClass {
  private readonly cliPath = join(import.meta.dir, "..", "..", "dist", "rig.js");
  private home = "";

  async run(): Promise<void> {
    this.home = await mkdtemp(join(tmpdir(), "rig-smoke-home-"));
    try {
      const env = { HOME: this.home, RIG_HOME: this.home };
      await this.rig(["init"], env);
      await this.rig(["create", "sample"], env);
      await this.rig(["list"], env);
      await this.rig(["ls"], env);
