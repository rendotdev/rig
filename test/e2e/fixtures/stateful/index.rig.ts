...
function defineStatefulTool(rig: Parameters<RigToolFactory>[0]) {
        ...
        run: async (context) => {
          ...
          if (context.input.action === "list") {
            return collection.list({
              where: context.input.whereStatus ? { status: context.input.whereStatus } : undefined,
      ...
      shell: rig.defineCommand({
        ...
        run: async (context) => {
          ...
          if (context.input.action === "json") {
            return context.rig.shell.json(["bun", "-e", "console.log(JSON.stringify({ok:true}))"]);
          }
          if (context.input.action === "bash") {
            return context.rig.shell.bash("printf bash-ok");
          }
          if (context.input.action === "bounded") {
            return context.rig.shell.exec(["bun", "-e", "process.stdout.write('🙂'.repeat(200))"], {
              maxOutputBytes: 17,
            });
          }
